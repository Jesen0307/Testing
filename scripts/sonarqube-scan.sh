...
# Prefer sonar.java.binaries when provided; otherwise use the binary output
# directory that Maven/Gradle typically produce.
BINARIES="$JAVA_BINARIES"
if [ -z "$BINARIES" ] && [ -d "$WORKSPACE_ROOT/target/classes" ]; then
